#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: NiagaraUIRenderer

#include "Basic.hpp"


namespace SDK::Params
{

// Function NiagaraUIRenderer.NiagaraSystemWidget.GetNiagaraComponent
// 0x0008 (0x0008 - 0x0000)
struct NiagaraSystemWidget_GetNiagaraComponent final
{
public:
	class UNiagaraUIComponent*                    ReturnValue;                                       // 0x0000(0x0008)(ExportObject, Parm, OutParm, ZeroConstructor, ReturnParm, InstancedReference, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(NiagaraSystemWidget_GetNiagaraComponent) == 0x000008, "Wrong alignment on NiagaraSystemWidget_GetNiagaraComponent");
static_assert(sizeof(NiagaraSystemWidget_GetNiagaraComponent) == 0x000008, "Wrong size on NiagaraSystemWidget_GetNiagaraComponent");
static_assert(offsetof(NiagaraSystemWidget_GetNiagaraComponent, ReturnValue) == 0x000000, "Member 'NiagaraSystemWidget_GetNiagaraComponent::ReturnValue' has a wrong offset!");

// Function NiagaraUIRenderer.NiagaraSystemWidget.HandleNiagaraSystemFinished
// 0x0008 (0x0008 - 0x0000)
struct NiagaraSystemWidget_HandleNiagaraSystemFinished final
{
public:
	class UNiagaraComponent*                      Comp;                                              // 0x0000(0x0008)(Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(NiagaraSystemWidget_HandleNiagaraSystemFinished) == 0x000008, "Wrong alignment on NiagaraSystemWidget_HandleNiagaraSystemFinished");
static_assert(sizeof(NiagaraSystemWidget_HandleNiagaraSystemFinished) == 0x000008, "Wrong size on NiagaraSystemWidget_HandleNiagaraSystemFinished");
static_assert(offsetof(NiagaraSystemWidget_HandleNiagaraSystemFinished, Comp) == 0x000000, "Member 'NiagaraSystemWidget_HandleNiagaraSystemFinished::Comp' has a wrong offset!");

// Function NiagaraUIRenderer.NiagaraSystemWidget.SetPlayParticle
// 0x0002 (0x0002 - 0x0000)
struct NiagaraSystemWidget_SetPlayParticle final
{
public:
	bool                                          InPlayParticle;                                    // 0x0000(0x0001)(Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
	bool                                          InResetOnActive;                                   // 0x0001(0x0001)(Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(NiagaraSystemWidget_SetPlayParticle) == 0x000001, "Wrong alignment on NiagaraSystemWidget_SetPlayParticle");
static_assert(sizeof(NiagaraSystemWidget_SetPlayParticle) == 0x000002, "Wrong size on NiagaraSystemWidget_SetPlayParticle");
static_assert(offsetof(NiagaraSystemWidget_SetPlayParticle, InPlayParticle) == 0x000000, "Member 'NiagaraSystemWidget_SetPlayParticle::InPlayParticle' has a wrong offset!");
static_assert(offsetof(NiagaraSystemWidget_SetPlayParticle, InResetOnActive) == 0x000001, "Member 'NiagaraSystemWidget_SetPlayParticle::InResetOnActive' has a wrong offset!");

// Function NiagaraUIRenderer.NiagaraSystemWidget.SetTickDependency
// 0x0008 (0x0008 - 0x0000)
struct NiagaraSystemWidget_SetTickDependency final
{
public:
	class AActor*                                 InNewActor;                                        // 0x0000(0x0008)(Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(NiagaraSystemWidget_SetTickDependency) == 0x000008, "Wrong alignment on NiagaraSystemWidget_SetTickDependency");
static_assert(sizeof(NiagaraSystemWidget_SetTickDependency) == 0x000008, "Wrong size on NiagaraSystemWidget_SetTickDependency");
static_assert(offsetof(NiagaraSystemWidget_SetTickDependency, InNewActor) == 0x000000, "Member 'NiagaraSystemWidget_SetTickDependency::InNewActor' has a wrong offset!");

// Function NiagaraUIRenderer.NiagaraSystemWidget.UpdateNiagaraSystemReference
// 0x0008 (0x0008 - 0x0000)
struct NiagaraSystemWidget_UpdateNiagaraSystemReference final
{
public:
	class UNiagaraSystem*                         NewNiagaraSystem;                                  // 0x0000(0x0008)(Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(NiagaraSystemWidget_UpdateNiagaraSystemReference) == 0x000008, "Wrong alignment on NiagaraSystemWidget_UpdateNiagaraSystemReference");
static_assert(sizeof(NiagaraSystemWidget_UpdateNiagaraSystemReference) == 0x000008, "Wrong size on NiagaraSystemWidget_UpdateNiagaraSystemReference");
static_assert(offsetof(NiagaraSystemWidget_UpdateNiagaraSystemReference, NewNiagaraSystem) == 0x000000, "Member 'NiagaraSystemWidget_UpdateNiagaraSystemReference::NewNiagaraSystem' has a wrong offset!");

// Function NiagaraUIRenderer.NiagaraSystemWidget.UpdateTickWhenPaused
// 0x0001 (0x0001 - 0x0000)
struct NiagaraSystemWidget_UpdateTickWhenPaused final
{
public:
	bool                                          NewTickWhenPaused;                                 // 0x0000(0x0001)(Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
};
static_assert(alignof(NiagaraSystemWidget_UpdateTickWhenPaused) == 0x000001, "Wrong alignment on NiagaraSystemWidget_UpdateTickWhenPaused");
static_assert(sizeof(NiagaraSystemWidget_UpdateTickWhenPaused) == 0x000001, "Wrong size on NiagaraSystemWidget_UpdateTickWhenPaused");
static_assert(offsetof(NiagaraSystemWidget_UpdateTickWhenPaused, NewTickWhenPaused) == 0x000000, "Member 'NiagaraSystemWidget_UpdateTickWhenPaused::NewTickWhenPaused' has a wrong offset!");

}

