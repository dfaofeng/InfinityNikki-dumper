...
{
public:
	static void ClearActiveTransitions(const struct FAnimUpdateContext& UpdateContext, const struct FAnimationStateMachineReference& Node);
...
public:
	static class UPlayMontageCallbackProxy* CreateProxyObjectForPlayMontage(class USkeletalMeshComponent* InSkeletalMeshComponent, class UAnimMontage* MontageToPlay, float PlayRate, float StartingPosition, class FName StartingSection, bool bMirror);
...
public:
	...
	static struct FSequencePlayerReference X6SetAccumulatedTime(const struct FSequencePlayerReference& SequencePlayer, float Time, bool bFireNotifies, class UAnimInstance* AnimInstance);
